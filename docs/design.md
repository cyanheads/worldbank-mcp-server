...
| `worldbank_get_poverty` | Poverty headcount, gap, and severity at any poverty line, plus the Gini coefficient, mean log deviation, polarization, and decile shares, from the Poverty and Inequality Platform. Individual economies only. | `countries`, `year`, `poverty_line`, `welfare_type`, `reporting_level`, `ppp_version`, `fill_gaps`, `page`, `per_page` | `readOnlyHint: true` |
...
|:--------|:------|:--------|
| `WorldBankApiService` | World Bank Indicators API v2 (`https://api.worldbank.org/v2/`), including its source-scoped data API (`/v2/sources/{id}/...`) | All tools and resources except `worldbank_get_poverty` and `worldbank_search_projects` |
...
| `WORLDBANK_DEFAULT_PER_PAGE` | No | Default page size for list/search operations (default: `50`). `worldbank_search_projects` and `worldbank_get_poverty` cap it like any requested size. |
| `WORLDBANK_CATALOG_CACHE_TTL_MS` | No | Lifetime of the in-process reference caches — the indicator catalog behind keyword-only search, the country index behind `isAggregate` and source-scoped country codes, each source-scoped dataset's `/concepts`, `/country`, `/time`, and dimension listings, and the PIP `/versions` listing behind `ppp_version` (default: `3600000`). `0` disables caching and refetches on every use. |
...
Zero observations are not an error — the tool returns an empty array with an `enrichment.notice`, matching `worldbank_search_indicators`. The notice is chosen from `total`: zero gets the broaden-the-query wording (naming the window when upstream dropped it), and an empty page of a non-empty result gets the page-past-end notice naming the pages that exist. `total` and `pages` describe the filtered observations on every page, the ones past the end included.
...
An empty page carries a `notice`: the filters that matched nothing when `total` is zero, or the pages that exist when the requested page is past the end. `format()` sees only the page, so its empty-page line (`No countries returned.`) makes neither claim.
...
- `page: number` (default: 1), `per_page: number` (default: 50, max: 1000) — applied locally; PIP has no pagination. A page holds at most 70 estimates (see the output-budget decision); a larger size, the server default included, is reduced with a notice and `pages` is counted at the reduced size.

**Output:**
- `estimates: Array<{ countryCode, countryName, regionCode, regionName, reportingYear, reportingLevel, welfareType, povertyLine, headcount, povertyGap, povertySeverity, watts, mean, median, gini, mld, polarization, decileShares, population, surveyYear, surveyAcronym, surveyComparability, comparableSpell, estimationType, isInterpolated }>` — ordered by country, year, reporting level, then welfare type.
...
Enrichment carries `appliedFilters` (including the applied `fillGaps`, the resolved `pppVersion` and `releaseVersion`, the served `perPage`, and `requestedPerPage` when that was reduced), `totalCount`, `currentPage`, `totalPages`, and a `notice` when the result is empty, the page is past the end, the page size was reduced, or the page contains gap-filled rows — composed into one string when more than one applies.
...
- `page: number` (default: 1), `per_page: number` (default: 50, max: 1000) — translated to the API's `os` (skip) and `rows`. A page holds at most 80 projects, or 8 with `include_abstract` (see the output-budget decision); a larger size, the server default included, is reduced with a notice, and both `os` and `pages` use the reduced size.
...
Enrichment carries `appliedFilters` (including `includeAbstract`, the served `perPage`, and `requestedPerPage` when that was reduced), `totalCount`, `currentPage`, `totalPages`, and a `notice` for an empty result, a page past the end, or a reduced page size.

**Errors:**
- `invalid_country_code` (`ValidationError`) — a `countries` entry is not two alphanumeric characters, most often an ISO3 code (`BRA`). Thrown before any request, ahead of the date checks, with the offending codes in `invalidCodes`. Upstream would answer an ISO3 code with a plain zero-hit. Recovery: use the ISO2 form, which `worldbank_get_country` reports as `iso2`.
...
- `upstream_unavailable` (`ServiceUnavailable`) — any non-success status or an HTML error page. A 4xx is marked non-retryable, since every filter is validated before the request and a rejection is therefore the API's own state rather than something a retry changes.
...
| Countries are two characters here, and an ISO3 code is rejected before the request as `invalid_country_code` | The Projects API keys on ISO2 (`BR`), where the rest of this server takes ISO3 (`BRA`); `countrycode_exact=BRA` returns `total: 0`. Since every other tool trains the agent on ISO3, this is the mistake most likely to be made, and the one that reads most convincingly as "no projects in Brazil". The handler rejects a three-letter code with a message naming the difference and a recovery hint pointing at `worldbank_get_country`, which reports the `iso2` field. It checks length rather than letters: 34 of the 218 codes in use are World Bank regional codes carrying a digit (`3A`, `4E`, `1W`), and a letters-only rule would make 1,073 multi-country operations unreachable. |
...
**Endpoints used:** `/pip?country=&year=&povline=&welfare_type=&reporting_level=&version=&fill_gaps=`, and `/versions` to resolve `version`. `country` takes one ISO3 code, a comma-separated batch, or `all`; `year` takes a year, `all`, or `MRV`. Codes are case-insensitive and unknown query parameters are ignored.
...
**Not exposed:** `/pip-grp` (regional and income-group aggregates), `/aux` (reference tables).
