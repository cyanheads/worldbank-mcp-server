...
| `WORLDBANK_CATALOG_CACHE_TTL_MS` | No | Lifetime of the in-process reference caches — the indicator catalog behind keyword-only search, the aggregate-code set behind `isAggregate`, and the PIP `/versions` listing behind `ppp_version` (default: `3600000`). `0` disables caching and refetches on every use. |
...
`indicator_not_queryable` has its own message id, `175` ("The indicator was not found. It may have been deleted or archived."), which upstream returns only after the country codes pass — a bad code on the same indicator produces the generic `120` and resolves as `country_not_found`. The id alone decides the reason. The service still makes the `/indicator/{id}` lookup so the message can name the indicator and its catalog source(s), but the lookup only adds detail: if it fails, the error is the same reason without the source.
...
- **Not every catalogued indicator can be queried for data.** The data endpoint rejects some indicators the catalog lists with message id `175`, and neither a country/date scope nor a `source` parameter changes that. Observed so far: the WDI Database Archives source (1,016 IDs under that source in `/indicator`; 17 sampled, all rejected), the 43 IDs published under both Food Prices for Nutrition and FPN Datahub Archive, and non-archive sources — sampled indicators from PEFA 2011, PEFA 2016, PEFA GRPFM, PEFA CRPFM, ICP 2005, ICP 2017, GDLD, and IDS: DSSI were all rejected. A source name is therefore no proxy for servability. `worldbank_search_indicators` and `worldbank_get_indicator` still return these IDs; `worldbank_get_data` reports them as `indicator_not_queryable`.
