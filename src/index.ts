...
await createApp({
  ...
  instructions:
    'Use the worldbank_* tools to query the World Bank Open Data API for development indicators. No API key required. Workflow: worldbank_search_indicators (browse topic/source IDs via worldbank_list_topics and worldbank_list_sources) to find an indicator_id, then worldbank_get_data to fetch values. Indicators use dotted codes like NY.GDP.PCAP.CD; countries use ISO2/ISO3, regional/income aggregate codes (EAS, HIC), WLD, or "all". On worldbank_get_data, date_range, mrv, and mrnev (each country\'s own latest values) are mutually exclusive; on series that publish quarterly or monthly values beside annual ones, frequency picks the form mrv and mrnev select from (annual by default) and a date_range\'s own form (2024Q1:2024Q4) picks it for a window; sparse cells return null, lastUpdated gives the data vintage, and "all" requires pagination. Poverty at a custom line, the Gini coefficient, and decile shares come from a separate dataset — use worldbank_get_poverty, which takes ISO3 or ISO2 economy codes plus PIP\'s own aggregates (WLD, regions such as SSF, income groups such as LIC, lending groups such as IDX) and returns null inequality fields on gap-filled years and aggregate rows. The Bank\'s lending operations are a third dataset — use worldbank_search_projects, which takes ISO3 or ISO2 economy codes but no aggregates, and combines its filters by AND.',
  tools: [
    worldbankListTopics,
